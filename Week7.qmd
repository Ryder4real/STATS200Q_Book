...
So, as we are dealing with frequentist Hypothesis Testing, our solution to maximize the numerator with respect to $\hat{theta}_{MLE}$. This means that we want to maximize the ratio $\frac{f_{\hat{\theta}_{MLE}}(x)}{f_{\theta_0}(x)}$.
